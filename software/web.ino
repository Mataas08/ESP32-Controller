...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    if (String(message.substring(0, 5)) == "F1___") {
      FACE_1 = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "F2___") {
      FACE_2 = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "F3___") {
      FACE_3 = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "F4___") {
      FACE_4 = message.substring(5, 6).toInt();
    }

    if (String(message.substring(0, 5)) == "D1___") {
      DPAD_UP = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "D2___") {
      DPAD_DOWN = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "D3___") {
      DPAD_LEFT = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "D4___") {
      DPAD_RIGHT = message.substring(5, 6).toInt();
    }

    if (String(message.substring(0, 5)) == "LS___") {
      LEFT_SHOULDER = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "RS___") {
      RIGHT_SHOULDER = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "LAB__") {
      LEFT_ANALOG_BUTTON = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "RAB__") {
      RIGHT_ANALOG_BUTTON = message.substring(5, 6).toInt();
    }

    if (String(message.substring(0, 5)) == "ST___") {
      START = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "SL___") {
      SELECT = message.substring(5, 6).toInt();
    }
    if (String(message.substring(0, 5)) == "H____") {
      HOME = message.substring(5, 6).toInt();
    }
...
void web() {
  ...
  server.on("/Joycon.js", HTTP_GET, [](AsyncWebServerRequest * request) {
    request->send(SPIFFS, "/Joycon.js", "text/css");
  });
}
